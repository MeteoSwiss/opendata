...
- The real-time daily analyses (preliminary) are available in items `CH at DD.MM.YYYY` with DD.MM.YYYY indicating the day for which the analysis is valid. The preliminary items are available for the past 41 days, with the oldest items being deleted successively.
- Consolidated daily and monthly analyses of the past 14 months are available in items `CH at MM.YYYY` with MM.YYYY indicating the month of interest. These items contain files with the daily analyses of all days of the month and files with single monthly analyses. The archive item for a specific month is established towards the end of the month following. The data will be published on the 10th and 26th of the following month, whereas the data from the 10th will be overwritten by the one from the 26th. Differences between the two versions are possible. Archive items older than 14 months are deleted successively.  
...
⚠️ **Please refer to the information in the info box above to see in which item you can find which files.**
