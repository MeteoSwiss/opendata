...
In practice for a given `reference_time` (_t_), both steps are available:

- **step = 0** provides the atmospheric state at time _t_  
- **step = 1** provides values aggregated over a one-hour period associated with _t_

This allows you to retrieve both instantaneous and time-aggregated information consistently for each hourly analysis.
