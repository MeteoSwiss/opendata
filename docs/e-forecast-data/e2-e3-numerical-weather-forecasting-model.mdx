...
**Variable parameters** change over time and include forecast quantities such as air pressure and temperature.

Static parameters describing the model grid and surface properties are provided separately. See [Model grid and static data](#model-grid-and-static-data).
...
Currently, the collection assets are not versioned, so there it is not possible to fetch previous versions of the static parameters. If you are creating an archive, you will need to keep track of the corresponding static files.
