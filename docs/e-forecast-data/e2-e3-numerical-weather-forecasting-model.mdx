...
- [Data structure](#data-structure)
    ...
    - [Model grid and static data](#model-grid-and-static-data)
        - [Static grid and surface files](#static-grid-and-surface-files)
...
- [ICON-CH1-EPS](https://data.geo.admin.ch/browser/#/collections/ch.meteoschweiz.ogd-forecasting-icon-ch1?.language=en)
- [ICON-CH2-EPS](https://data.geo.admin.ch/browser/#/collections/ch.meteoschweiz.ogd-forecasting-icon-ch2?.language=en)
- [KENDA-CH1](https://data.geo.admin.ch/browser/index.html#/collections/ch.meteoschweiz.ogd-analysis-kenda-ch1?.language=en).
...
If a specific pollen variable cannot be found, it is likely that the corresponding 
pollen type is **not currently in season**. To verify which pollen parameters are 
currently available, please consult the [parameter overview](#parameter-overview) file mentioned above.
...
<Tabs queryString="download-options" groupId="download-options">
    ...
    - `collections`: Defines the forecast or analysis model to retrieve (`ch.meteoschweiz.ogd-forecasting-icon-ch1` for ICON-CH1-EPS, `ch.meteoschweiz.ogd-forecasting-icon-ch2` for ICON-CH2-EPS and `ch.meteoschweiz.ogd-analysis-kenda-ch1` for KENDA-CH1).
    ...
    **Steps:**
    ...
    In addition to the forecast files, each collection contains a vertical constants file and a horizontal constants file. See [Static grid and surface files](#static-grid-and-surface-files) for their contents.
...
### Model grid and static data {#model-grid-and-static-data}
The model data is structured on both a horizontal and vertical grid. Static parameters describing the grid and surface properties are provided in separate GRIB2 files.

#### Static grid and surface files {#static-grid-and-surface-files}

The static parameters remain fixed within a single model run. Each ICON collection provides two static files:

- Vertical constants:
    - **HHL** Geometric height of the layer limits above sea level, see the [vertical grid documentation](#vertical-grid)
...
- Horizontal constants:
    ...
    - **SOILTYP** Type of soil, based on the [soilType.table](https://github.com/COSMO-ORG/eccodes-cosmo-resources/blob/master/definitions/soilType.table)
...
A universally unique identifier (UUID) is assigned to both horizontal and vertical constants, distinguished as `uuidOfHGrid` and `uuidOfVGrid`, to verify that the static data is consistent with the forecast data.

:::

For a practical example of how to download static parameters in Python, refer to notebook [09_constant_parameters.ipynb](https://github.com/MeteoSwiss/opendata-nwp-demos/blob/main/09_constant_parameters.ipynb).

:::info

When downloading variable parameters through the Python API, the corresponding horizontal coordinates (`CLON`, `CLAT`) are automatically included.

:::

:::warning

Currently, the collection assets are not versioned, so it is not possible to fetch previous versions of the static parameters. If you are creating an archive, you will need to keep track of the corresponding static files.
...
<details>
<summary> <b> What if I see incomplete data? </b> </summary>

In the case of a problem with a forecast run there could be missing data or a delay in the upload of the data.
</details>
...
For more information, refer to notebook [09_constant_parameters.ipynb](https://github.com/MeteoSwiss/opendata-nwp-demos/blob/main/09_constant_parameters.ipynb) or read section [Accessing static grid information: Height, longitude, latitude and surface properties](/e-forecast-data/e2-e3-numerical-weather-forecasting-model?download-options=restapi#accessing-static-grid-information-height-longitude-latitude-and-surface-properties).
</details>
...
<details>
<summary> <b> How can I access KENDA analysis data? </b> </summary>

Analysis data generated by [KENDA](https://www.meteoswiss.admin.ch/weather/warning-and-forecasting-systems/icon-forecasting-systems/ensemble-data-assimilation.html) is available through the Open Data interface. For more details, see [Numerical weather analysis KENDA-CH1](https://opendatadocs.meteoswiss.ch/e-forecast-data/e5-numerical-weather-analysis-data).

Alternatively, you can use the **forecast data of ICON-CH1-EPS or ICON-CH2-EPS at lead time 0** (*horizon 0*), which provides fields equivalent to the analysis.  
These are available every 3 hours (ICON-CH1-EPS) or 6 hours (ICON-CH2-EPS), instead of hourly as with KENDA.
</details>

## Upcoming changes and changelog {#changes}

[Here](https://opendatadocs.meteoswiss.ch/e-forecast-data/e2-e3-numerical-weather-forecasting-model-changelog) you will find information about upcoming changes and the changelog.
