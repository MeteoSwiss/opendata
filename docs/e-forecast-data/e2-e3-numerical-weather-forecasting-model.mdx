...
Users can find information about available parameters, including metadata about height, longitude and latitude, in the collections decribed above. To access the parameter overview click on the link for the desired model. A new browser window will open; then, navigte to the `Assets` section and click the download button.
...
- Each GRIB file contains data for a single model collection, one reference time, one forecast lead time, and one variable — either as the deterministic control run or the full ensemble.
- Diagram
...
<Tabs queryString="data-structure">
    ...
    <TabItem value="retrieving-forecasts-via-rest-api" label="Retrieving forecasts via REST API">
...
To explore them click on the following GitHub Repository button.
...
- ["Why can't I find latitude/longitude?"](#-why-cant-i-find-latitudelonitude)
- ["Why does my download expire?"](#why-does-my-downloa-expire)
- ["How to interpret different types of levels?"](#how-to-interpret-different-types-of-levels)

### "Why can't I find latitude/longitude?"
