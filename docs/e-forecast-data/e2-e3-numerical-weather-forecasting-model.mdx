...
The provided GRIB files consistently follow the same structure. Each GRIB file contains data for a single model collection, one reference time, one forecast lead time, and one variable — either as the deterministic control run or the full ensemble. The following diagram shows a general overview.
...
The data provided in the two collections described in the [Model specifications table](#model-specifications) is accessible for only **24 hours**. After 24 hour the data expire.
</details>
