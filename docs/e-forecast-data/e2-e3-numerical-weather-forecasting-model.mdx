...
<Tabs queryString="download-options" groupId="download-options">
    ...
    **Steps:**
    ...
    Besides the current forecast files, each collection contains two static files. The *vertical constants* file stores information about the height of the half levels (HHL) in the vertical grid. The *horizontal constants* file contains the following parameters.
    - CLON: The longitude of the center point coordinates of each triangle on the horizontal grid
    - CLAT: The latitude of the center point coordinates of each triangle on the horizontal grid
    ...
    - SOILTYPE: Type of soil (1 to 9 corresponding to the local soilType.table)
