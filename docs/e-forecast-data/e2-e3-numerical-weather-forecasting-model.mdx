...
At the moment, the collection assets are not versioned, so there is no way to fetch a previous version of the static parameters. If you are building an archive, you may need to keep track of the corresponding static files.
