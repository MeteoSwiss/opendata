...
<Tabs queryString="download-options" groupId="download-options">
    ...
    **Steps:**
    ...
    Besides the current forecast files, each collection contains two static files: the vertical constants file, which stores information about the heights of the half-levels (HHL) in the vertical grid, and the horizontal constants file, which contains the following parameters.
    - CLON: The longitude of the center point coordinate of each triangle on the horizontal grid
    - CLAT: The latitude of the center point coordinate of each triangle on the horizontal grid
    - FR_LAND: The land cover (1 if there is land, 0 if there is the sea)
    - HSURF: Geometric height of the earths surface above sea level
    - SOILTYPE: Type of soil according to the [soilType.table](https://github.com/COSMO-ORG/eccodes-cosmo-resources/blob/master/definitions/soilType.table)
    ...
    Once the static GRIB file is downloaded, you may want to associate a value from a data file (for a given parameter) to its latitude and longitutde coordinate.
    To achieve this, ensure that the `uuidOfHGrid` (Universally Unique Identifier for the horizontal grid) key in the data file matches the one in the parameters `CLON` and `CLAT` of the the static horizontal file.
    Then, add the associated coordinates to the values from the data file on a point-by-point basis.
...
For more information, read section [Accessing static grid information: Height, longitude and latitude](/e-forecast-data/e2-e3-numerical-weather-forecasting-model?download-options=restapi#accessing-static-grid-information-height-longitude-and-latitude).
</details>
